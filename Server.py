...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS flatfoot_results
                 (timestamp TEXT, probability_normal REAL, probability_flat REAL, user_id TEXT, 
                  n_windows INTEGER, model_source TEXT)''')
    conn.commit()
    conn.close()
...
try:
    if os.path.exists("V02_Infer.keras"):
        ...
        test_output = flatfoot_model.predict(test_input, verbose=0)
        print(f"✅ Flatfoot 模型測試成功: 輸出形狀 {test_output.shape}")
    else:
...
print(f"模型載入狀態: Flatfoot={flatfoot_model_loaded}")
print("=" * 60)

# --- 步態事件檢測函數 (使用 scipy 替代 gait_model_5.h5) ---
def detect_gait_events_scipy(test_time, test_gyro, threshold=1.0, distance=20):
    """使用 scipy 的峰值檢測來替代 gait_model_5.h5"""
    try:
        # 標準化數據
        gz_normalized = (test_gyro - np.mean(test_gyro)) / np.std(test_gyro)
        
        # 尋找峰值 (TO 事件)
        peaks, _ = find_peaks(gz_normalized, height=threshold, distance=distance)
        
        print(f"Scipy 檢測到 {len(peaks)} 個 TO 事件")
        
        # 簡單的 HS 事件檢測：在 TO 事件之間找最大值
        hs_indices = []
        for i in range(len(peaks) - 1):
            start_idx = peaks[i]
            end_idx = peaks[i + 1]
            
            if end_idx - start_idx > 10:  # 確保有足夠的間隔
                segment = test_gyro[start_idx:end_idx]
                hs_idx = start_idx + np.argmax(segment)
                hs_indices.append(hs_idx)
        
        print(f"Scipy 檢測到 {len(hs_indices)} 個 HS 事件")
        
        # 組合結果
        pred_events = []
        for idx in peaks:
            pred_events.append((test_time[idx], "TO"))
        for idx in hs_indices:
            pred_events.append((test_time[idx], "HS"))
            
        return pred_events
        
    except Exception as e:
        print(f"Scipy 事件檢測失敗: {e}")
        return []
...
def preprocess_flatfoot_data_from_csv(csv_content: str) -> Tuple[np.ndarray, int]:
    ...
    try:
        ...
        # 使用 scipy 檢測 TO 事件 (替代 gait_model_5.h5)
        to_events = detect_gait_events_scipy(time_data, gz)
        to_indices = [int(event[0]) for event in to_events if event[1] == "TO"]
        print(f"檢測到 {len(to_indices)} 個 TO 事件")
...
def predict_flatfoot(X_windows):
    ...
    if not flatfoot_model_loaded or len(X_windows) == 0:
        return None, None, "model_not_loaded"
    ...
    try:
        ...
        return p_normal_mean, p_flat_mean, "real_model"
...
async def predict_from_csv(file: UploadFile = File(...)):
    ...
    try:
        ...
        # 使用 scipy 替代 gait_model_5.h5
        results = detect_gait_events_scipy(test_time, test_gyro)
...
async def analyze_flatfoot(file: UploadFile = File(...)):
    ...
    try:
        ...
        # 進行預測
        p_normal_mean, p_flat_mean, model_source = predict_flatfoot(X_windows)
        
        if p_normal_mean is None:
        ...
        c.execute("INSERT INTO flatfoot_results VALUES (?,?,?,?,?,?)",
                  (datetime.now().isoformat(), p_normal_mean, p_flat_mean, 
                   "current_user", len(X_windows), model_source))
        conn.commit()
        conn.close()
        ...
        result_data = {
            "timestamp": datetime.now().isoformat(),
            "probability_normal": round(p_normal_mean, 4),
            "probability_flat": round(p_flat_mean, 4)
        }
...
async def get_flatfoot_results(limit: int = 10):
    ...
    try:
        conn = sqlite3.connect('gait_results.db')
        c = conn.cursor()
        c.execute("SELECT timestamp, probability_normal, probability_flat, n_windows FROM flatfoot_results ORDER BY timestamp DESC LIMIT ?", (limit,))
        
        results = [{
            "timestamp": r[0], 
            "probability_normal": r[1],
            "probability_flat": r[2],
