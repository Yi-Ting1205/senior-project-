...
# Initialize Flask
app = Flask(__name__)
app.config['MAX_CONTENT_LENGTH'] = 1 * 1024 * 1024  # Limit upload to 1MB
...
# Load models (place these files in the same directory)
try:
    phase_model = load_model("gait_phase_model.h5")  # For stance/swing phases
    class_model = load_model("foot_type_model.h5")   # For flat foot detection
    logging.info("Models loaded successfully")
except Exception as e:
    logging.error(f"Model loading failed: {str(e)}")
    raise

def predict_events(time_data, gyro_data):
    """Mock event prediction - replace with your actual model logic"""
    events = []
    for i in range(1, len(time_data)):
        if i % 30 == 0:
            events.append((float(time_data[i]), "HS"))  # Heel strike
        elif i % 20 == 0:
            events.append((float(time_data[i]), "TO"))  # Toe off
    return events

def calculate_phases(events):
    """Calculate stance/swing phases from events"""
    if not events:
        return {"Stance Phase": 0.6, "Swing Phase": 0.4}
    
    hs_times = [t for t, e in events if e == "HS"]
    to_times = [t for t, e in events if e == "TO"]
    ...
    stance_durations = []
    for i in range(len(hs_times)-1):
        hs1, hs2 = hs_times[i], hs_times[i+1]
        cycle_to = [to for to in to_times if hs1 < to < hs2]
        if cycle_to:
            stance_durations.append((cycle_to[0] - hs1) / (hs2 - hs1))
    ...
    return {
        "Stance Phase": float(avg_stance),
        "Swing Phase": float(1 - avg_stance)
    }

def predict_foot_type(df):
    """Mock classification - replace with your actual model"""
    return 0.75  # Simulated 75% probability of flat foot
...
def predict():
    ...
    try:
        ...
        # Validate columns
        required_cols = ["Time", "Gyroscope_Z"] + FEAT_COLS
        ...
        gyro_data = df["Gyroscope_Z"].values
        
        events = predict_events(time_data, gyro_data)
        phases = calculate_phases(events)
        flat_prob = predict_foot_type(df)
        ...
        return jsonify({
            ...
            "error": None,
            "analysis": {
                "Stance Phase": phases["Stance Phase"],
                "Swing Phase": phases["Swing Phase"],
                "IsFlatFooted": flat_prob > 0.5,
                "FlatFootProbability": flat_prob
            },
            "events": [{
                "time": float(t),
                "event": e
